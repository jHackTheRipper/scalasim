...
object IsingModelSeq {
  ...
  def lattice(latticeSize : Int, threshold : Double)(implicit rng: Random) =
    new Lattice {
      ...
      val lattice = (0 to latticeSize * latticeSize - 1).map{i => (i % latticeSize -> i / latticeSize) -> (rng.nextDouble < threshold)}.toIndexedSeq
    }
...
class IsingModelSeq {
  ...
  new Lattice {
    ...
    val lattice = 
      IndexedSeq.concat (
        _lattice.filter{case((x, y), _) => isEven(x, y)}.map(spin => processSpin(_lattice, spin)),
        _lattice.filter{case((x, y), _) => isOdd(x, y)}.map(spin =>  processSpin(_lattice, spin))
      )
  }
